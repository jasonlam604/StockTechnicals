...
public class SimpleMovingAverage {
	...
	/**
	 *  Simple Moving Average is formed by computing the average price of a security over 
	 *  a specific number of periods. 
	 *  
	 *  Calculation starts from index 0 to index n assuming there is enough periods
	 * 
	 * @param price double array of values
	 * @param period SMA period to be calculated over
	 * @throws Exception
	 */
	public double[] execute(double[] price,int period) throws Exception {
		...
		for(int i=0; i<= maxLength; i++) {
			results[i] = NumberHelper.roundTwoDecimals((Arrays.stream(Arrays.copyOfRange(price, i, (i+period))).sum()) /  period);
		}
